...
const (
	Add_Add_FullMethodName = "/bilibili.broadcast.v1.Add/Add"
)
...
type AddClient interface {
	Add(ctx context.Context, opts ...grpc.CallOption) (Add_AddClient, error)
}
...
func (c *addClient) Add(ctx context.Context, opts ...grpc.CallOption) (Add_AddClient, error) {
	stream, err := c.cc.NewStream(ctx, &Add_ServiceDesc.Streams[0], Add_Add_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
	x := &addAddClient{stream}
	return x, nil
}

type Add_AddClient interface {
	Send(*AddParams) error
	Recv() (*AddResult, error)
	grpc.ClientStream
}
...
func (x *addAddClient) Send(m *AddParams) error {
	return x.ClientStream.SendMsg(m)
}

func (x *addAddClient) Recv() (*AddResult, error) {
	m := new(AddResult)
...
func (UnimplementedAddServer) Add(Add_AddServer) error {
...
func _Add_Add_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(AddServer).Add(&addAddServer{stream})
}

type Add_AddServer interface {
	Send(*AddResult) error
	Recv() (*AddParams, error)
	grpc.ServerStream
}

type addAddServer struct {
	grpc.ServerStream
}

func (x *addAddServer) Send(m *AddResult) error {
	return x.ServerStream.SendMsg(m)
}

func (x *addAddServer) Recv() (*AddParams, error) {
	m := new(AddParams)
...
var Add_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "bilibili.broadcast.v1.Add",
